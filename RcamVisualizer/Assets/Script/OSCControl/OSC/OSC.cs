...
//{
  ...
  public class OSC : MonoBehaviour
  {
    ...
    public void InitializeOSC() {
        ...
        Debug.Log("Opening OSC sender on IP " + outIP + " and port " + outPort + ". Listening on port " + inPort);
	...
	public string SetAddressHandler(string key, OscMessageHandler ah)
	{
        // UberDebug.LogChannel("OSC", "Setting address handler for key " + key);
    ...
    public void RemoveAddressHandlerForKey(string key)
    {
        ArrayList al = (ArrayList)Hashtable.Synchronized(AddressTable)[key];
        if (al != null)
        {
            Hashtable.Synchronized(AddressTable).Remove(key);
        }
    }
